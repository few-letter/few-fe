...
│   └── /utils              # 유틸리티 함수
├── /config                 # 환경 변수 설정 (env.ts)
├── /fonts                  # 폰트 설정 (Pretendard, Numans)
├── /lib                    # 외부 라이브러리 통합 (Mixpanel, utils)
...
NEXT_PUBLIC_ENABLE_MIXPANEL      # Mixpanel 활성화 여부 ("true" / "false")
```
...
API_ROUTES = {
  ...
  CONTENT_TYPES: "/api/v1/contents/types",
